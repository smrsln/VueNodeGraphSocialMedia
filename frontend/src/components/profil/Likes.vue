      ...
      <div class="row">
        ...
        <div class="col col-xl-6 order-xl-2 col-lg-12 order-lg-1 col-md-12 col-sm-12 col-12">
          <div id="newsfeed-items-grid">
             <div class="alert alert-secondary" v-if="!(data.length > 0)" role="alert">
                Begendiğiniz bir paylaşım bulunmamaktadır.
             </div>
...
export default {
  ...
  methods: {
    getFollowers(userid) {
      //console.log(userid);
      apiService
        .getMyFavoriteEtries(userid,this.$cookies.get("user").id, this.pg)
        .then(data => {
          if(data == "0"){
            this.data = {};
          }
    ...
    dislikeControl(entryId, userDislike) {
      ...
      if (userDislike > 0) {
        ...
        apiService.dislikeService(item).then(data => {
          if (data.data === "ok") {
            this.getFollowers(tthis.profilUserId);
            toastr.success(
  ...
  mounted() {
    this.pg = 1;
    this.getFollowers(this.profilUserId);//this.$cookies.get("user").id)
  },
  computed: {
    profilUserId() {
